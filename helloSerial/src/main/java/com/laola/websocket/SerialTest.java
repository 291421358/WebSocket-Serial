...
public class SerialTest extends Thread implements SerialPortEventListener { // SerialPortEventListener
                ...
                try {
                    ...
                    while (inputStream.available() > 0) {
                        ...
                        if (numBytes > 0) {
                            ...
                            for (byte read :readBuffer
                                 ) {
                                System.out.print(read + " ");
                            }
                            WebSocket webSocket = new WebSocket();
                            webSocket.sendMessageAll(new String(readBuffer));
                            readBuffer = new byte[10];// 重新构造缓冲对象，否则有可能会影响接下来接收的数据
                        } else {
...
//                }
    ...
    public static void main(String[] args) {
        SerialTest cRead = new SerialTest();
        ...
        if (null != serialPort) {
            ...
            try {
                ...
                byte[] bytes = hexStrToBinaryStr(st);
