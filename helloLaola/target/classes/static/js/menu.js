	...
	var menu = {
		getMenu: function(curl) {
			...
			$.getJSON(curl, function(res) {
				...
				var items = [];
				var _ul = $('<ul style="text-align: center"></ul>').addClass('nav').attr('id', 'nav');
				$.each(menuList, function(index, val) {
					var item = '<li  id="menu' + val.id + '"><a _href="' + val.url + '"><i class="iconfont" style="font-size: 40px; color: red; float: none">' + val.icon +
						'</i><br/><cite style="margin-left: 11px">' + val.name +
